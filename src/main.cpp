...
#if defined(NDEBUG)
# error "Sheetgold cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd7e85ae1efad3ab41fd6b3e72189e675461490d469bfd6ad140d942f23ea48c6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sheetgold: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sheetgold Signed Message:\n";
...
{
    // Sheetgold: IsDust() detection disabled, allows any valid dust to be relayed.
  ...
  int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    else if(nHeight < 10000)
   {
    ...
    nSubsidy >>= (nHeight / 840000); // Sheetgold: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Sheetgold: 1 day
static const int64 nTargetSpacing = 1 * 30; // Sheetgold: 30  seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sheetgold: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sheetgold: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd7e85ae1efad3ab41fd6b3e72189e675461490d469bfd6ad140d942f23ea48c6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Welcome to SheetGold";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("1716e1710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528476704;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1547229;
        ...
        assert(block.hashMerkleRoot == uint256("0xd3beb1f930ec9b2d5e47f59b337045ae3d118b6c8116c7c432801e1b25b0c93c"));
...
unsigned char pchMessageStart[4] = { 0xe1, 0x16, 0xd3, 0xd4 }; // Sheetgold: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SheetgoldMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SheetgoldMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SheetgoldMiner(CWallet *pwallet)
{
    printf("SheetgoldMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sheetgold-miner");
    ...
    try { loop {
        ...
        printf("Running SheetgoldMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SheetgoldMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SheetgoldMiner, pwallet));
}
